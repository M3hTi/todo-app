...
- **Rust side (`src-tauri/src/lib.rs`):** registers plugins (sql, notification, dialog, fs, opener, autostart) and owns *only* the native shell — tray icon/menu/tooltip, close-to-tray window interception, and the `update_tray`/`quit_app` commands. It holds no task/business logic; the frontend pushes tray state in and decides close behavior.
