...
- **Releases:** tag `vX.Y.Z` → `.github/workflows/release.yml` builds, signs and drafts a GitHub Release with `latest.json`; the in-app updater (`src/lib/updater.ts`) polls it on launch. Signing needs the `TAURI_SIGNING_PRIVATE_KEY` / `..._PASSWORD` repo secrets. The repo must stay **public** for the updater to reach that feed — the download URL 404s unauthenticated. `.github/workflows/ci.yml` typechecks and tests every push/PR.
